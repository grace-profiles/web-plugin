...
But if you want only show it's own commands and features, you can execute `grace profile-info web-plugin --only`. 
You will see that `web-plugin` profile extends from `base` and `plugin`, but it doesn't provides any commands and features.
...
Compared to the `plugin` profile, the `web-plugin` provides `asset-pipeline-plugin` and `gsp` in addition to `maven-publish` by default.
